...
function addToTagList(e) {
    ...
    Array.from(tagList.children).forEach(tag => {
        if (tag === e.currentTarget) contains = 1;
    });

    if (contains) {
        buttonsList.append(e.currentTarget);
        Array.from(workingArea.children).forEach(tag => {
            if (tag.innerText === e.currentTarget.innerHTML) tag.remove();
            console.log(tag.innerText);
        });
    }
    else {
        tagList.append(e.currentTarget);
        Array.from(workingArea.children).forEach(tag => {
            if (tag.classList.contains('tag')) top += 1.3;
        });
        workingArea.innerHTML += `
            <div class="tag width_full flex_row justify_center" style="color: #FFF;">
                <p>Tag</p>
            </div>
        `;

        const tag = document.querySelector('.tag.width_full.flex_row.justify_center:last-child');
        tag.style.position = 'absolute';
        tag.innerHTML = e.currentTarget.innerHTML;
        tag.style.top = `${ top }rem`;
        tag.style.left = '30rem'
        tag.style.width = '20%';
        tag.style.background = e.currentTarget.style.background;
    }
}
